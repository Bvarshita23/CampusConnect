...
export default function BulkUploadFaculty() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("duplicateMode", "replace");

      const res = await axios.post(
        "/api/v1/auth/bulk-upload/faculty",
        formData,
        {
          headers: {
      ...
      alert(
        err.response?.data?.message ||
          "Bulk faculty upload failed. Check console."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 flex justify-center">
      <div className="bg-white w-full max-w-xl shadow-xl p-8 rounded-2xl">
        <h2 className="text-3xl font-bold text-center mb-3 text-purple-700">
          Bulk Upload Faculty
        </h2>
        ...
        <button
          onClick={() => navigate(-1)}
          className="block mx-auto mt-6 text-purple-700 hover:underline"
        >
          ← Go Back
        </button>
      </div>
